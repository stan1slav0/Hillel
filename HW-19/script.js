...
getMoney()
	.then(
			() => {
					...
					do {
							currency = prompt(`Оберіть валюту: ${userCurrency.join(`, `)}`, userCurrency[0]);
			...
			() => {
					...
					do {
							currency = prompt(`Оберіть валюту: ${availableCurrencies.join(`, `)}`, availableCurrencies[0]);
							if (currency) currency = currency.replaceAll(` `, ``).toUpperCase();
					} while (!userCurrency.includes(currency))
					...
					if (amountMoney > maxBankMoney) console.log(`Введена сума більша за доступну. Максимальна сума зняття: ${maxBankMoney}`);
					else if (amountMoney < minBankMoney) console.log(`Введена сума менша за доступну. Мінімальна сума зняття: ${minBankMoney}`);
					else if (amountMoney) console.log(`От ваші гроші: ${amountMoney}${imgBankMoney}`);
			}
	)
	.finally(
			() => {
